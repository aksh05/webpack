 ...
 * Author : Ankit Anand
 * Version : v1
 * Description : Fixed param undefined reference bug in this.upload function
 */

(function($){

window.ncUploader = function(params) {
    ...
    var construct = function(me) {
        ...
        $('#' + me.params.fileId).unbind('change').on('change', uploader.upload);
    ...
    this.deleteFile = function(fileKey, fileName) {
        var delData = {
            'appId': params.appId,
...
//Edited by Tarun(Start)
    ...
    function delFunc(data, textStatus, xhr) {
      //console.log(typeof data + data);  
        if (data && typeof data[fileKey].ERROR !== "undefined") {
          callbackArguments = {
            ERROR: {}
          };
          ...
          callbackArguments.SUCCESS[delData['fileKey']] = {
            name: fileName,
...
//Edited by Tarun(End)
    ...
    this.pluginCallback = function(type, args) {
        ...
        if (type === "success") {
            ...
            updateCount("inc");
            fileKeys.push(args.fileKey);
            //TODO make ajax call to delete file on server
            deleteFile(args.fileKey, args.name);
        }
    };


    var updateCount = function(action) {
        if (action == "inc") {
            uploadsLeft++;
            if (uploadsLeft === 1) {
                for (var pluginName in params.plugins) {
                ...
                $('#' + me.params.fileId).show();
            }
            if (uploadsLeft === params.maxNumOfFiles) {
                $("#" + me.params.outputHolder.id).hide();
            }
...
var ncBasicUploader = function() {
    ...
    var createUploadForm = function() {
        ...
        $(fileObj.clone(true)).insertBefore(fileObj.nextSibling);
...
var ncAdvancedUploder = function() {
    ...
    this.upload = function() {
        if (me.params.maxNumOfFiles !== 1 && me.isUploadAllowed() == false) {
    ...
    var sendFile = function(data, file) {
        ...
        $.ajaxq("uploadqueue", {
            ...
            beforeSend: function() {
              ...
              //edit by prashu
                if (me.params.outputHolder) $("#" + me.params.outputHolder.id).html(res);
            ...
            xhr: function() {
                ...
                if (myXhr.upload) {
                    ...
                    myXhr.upload.addEventListener('progress', progressHandlerCreator($('#' + rno).children('progress')), false);
    ...
    this.serverHit = function(fileInfo) {
        ...
        $.ajaxq("uploadqueue", {
            ...
            beforeSend: function(jqXhr, obj) {
              ...
              // edit by prashu
                if (params.outputHolder) $("#" + params.outputHolder.id).html(res);
                fK = getFileKey();
