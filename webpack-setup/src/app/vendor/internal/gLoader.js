$.fn.gLoader = function(){
	var prv = {
		...
		appendGL : function(){
			...
			var mLoader = '<div class="preloader-wrapper big active valign" style="margin:0 auto;"><div class="spinner-layer spinner-blue-only"><div class="circle-clipper left"><div class="circle"></div></div><div class="gap-patch"><div class="circle"></div></div><div class="circle-clipper right"><div class="circle"></div></div></div></div>';
				
			t.meta.obj.each(function(){
				var c = $(this),cW = t.objW(c), cH = t.objH(c);
				if(c.find('#gL').length!==0)return;
				var currLCont = lContHtml.html(mLoader).clone();
				t.setClass(c,currLCont);
				c.attr({'cssProp':c.css('position')+','+c.css('overflow')});
				c.css({'position':'relative','overflow':'hidden'}).append(currLCont.css({'width':cW+t.srlW+'px','height':cH+'px',top:t.scrollTop(c)+'px'}).show());
				//currLCont.show();				
				currLCont.css({'display':'flex'});
