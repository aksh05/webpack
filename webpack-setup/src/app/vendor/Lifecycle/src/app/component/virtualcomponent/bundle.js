var something = (function () {
	...
	*/
	...
	var renderFn = function (previous, current) {
	    ...
	    component.props = _extends({}, component.props, current.args);
	...
	*/
	...
	var JobTuple = function (_VirtualComponent) {
	    ...
	    JobTuple.prototype.shouldComponentUpdate = function shouldComponentUpdate(nextProps, nextState) {
	        return this.props.tuple != nextProps.tuple || this.state != nextState;
	    };

	    JobTuple.prototype.getInitialState = function getInitialState() {
	        return {
	            isSaved: false
	        };
	    };

	    JobTuple.prototype.reducer = function reducer() {
	        var state = arguments.length <= 0 || arguments[0] === undefined ? {} : arguments[0];
	        var action = arguments[1];

	        switch (action.type) {
	            case 'TUPLE_SAVE':
	                return _extends({}, state, { "isSaved": !state.isSaved });
	            default:
	                return state;
	        }
	    };

	    JobTuple.prototype.toggleSave = function toggleSave() {
	    ...
	    JobTuple.prototype.render = function render() {
	        ...
	        return hyperScript(
	            "article",
	            null,
	            hyperScript("a", { className: isJobSaved }),
	            hyperScript(
	                "a",
	                null,
	                hyperScript(
	                    "span",
	                    { className: "title" },
	                    this.props.tuple.title
	                ),
	                hyperScript(
	                    "span",
	                    { className: "cName" },
	                    this.props.tuple.desc
	                ),
	                hyperScript(
	                    "b",
	                    null,
	                    this.props.tuple.experience,
	                    " Years"
	                ),
	                hyperScript(
	                    "b",
	                    null,
	                    this.props.tuple.location
	                ),
	                hyperScript(
	                    "b",
	                    { className: "ellipsis" },
	                    this.props.tuple.keySkills
	                )
	            )
	        );
	    };
	    ...
	    JobTuple.prototype.attachedCallback = function attachedCallback() {
	        ...
	        $(this.node).find(".starIc").on("click", function (event) {
	            _this2.toggleSave();
	        });
	    };
	...
	var TupleList = function (_HTMLElement) {
	    inherits(TupleList, _HTMLElement);

	    function TupleList() {
	        classCallCheck(this, TupleList);
	        return possibleConstructorReturn(this, _HTMLElement.apply(this, arguments));
	    }

	    TupleList.prototype.render = function render(tuples, store) {
	        return hyperScript(
	            "div",
	            null,
	            hyperScript(
	                ...
	                tuples.map(function (tuple, index) {
	                    return hyperScript(JobTuple, { tuple: tuple, store: store, index: index, transition: transition });
	                }),
	                hyperScript(
	                    "a",
	    ...
	    TupleList.prototype.reducer = function reducer() {
	        var state = arguments.length <= 0 || arguments[0] === undefined ? [] : arguments[0];
	        var action = arguments[1];

	        switch (action.type) {
	            case 'ADD_TUPLES':
	                return state.concat(action.tuples);
	            case 'TUPLE_SELECT':
	                return state.map(function (value, index) {
	                    if (index == action.index) {
	                        return _extends({}, value, { isSelected: !value.isSelected });
	                    }
	                    return value;
	                });
	            default:
	    ...
	    TupleList.prototype.createdCallback = function createdCallback() {
	        var state = [{
	            desc: "Oracle",
	            experience: "2-5",
	            isSelected: false,
	        ...
	        store = createStore(this.reducer, state, applyMiddleware(logger));

	        var batcher = batch(store.getState(), this.render);
	        this.appendChild(batcher.rootNode);
	        store.subscribe(function () {
	            batcher.update(store.getState());
	        });
	    };

	    TupleList.prototype.attachedCallback = function attachedCallback() {
	        $(this).find("#nextPage").on("click", function () {
	            store.dispatch({
	                type: "ADD_TUPLES",
	                tuples: [{
	                    desc: "Oracle",
	...
	document.registerElement("tuple-list", TupleList);
